...
| 0.8B and 2B transport works but agent file-write behavior is unreliable | Community/model | Medium | Packed 0.19 live runs and three 2B trials; previous filesystem checks | Separate protocol/plugin/task results in model report | Implemented | 4B and models above 9B were not loaded |
| 9B exact-answer latency is not consistently bounded by Hermes cancellation | Hermes/QVAC/model | High | Fresh protocol suite passed; two Hermes smokes exceeded 75 seconds and cancellation left QVAC busy | Preserve as a cross-boundary limitation; do not claim deterministic smoke quality | Reproducible | Root ownership of request cancellation needs upstream review |
